...
def analyze_star(
    ...
    *,
    ...
    """
    Output keys (compatible with your scan):
    ...
    try:
        ...
        # if psi has NaNs => analysis unreliable
        ...
        # If we have X-points, attempt separatrix at psi_sep = psi(xpoint)
        psi_sep = None
        if xps:
        ...
        if psi_sep is not None and np.isfinite(psi_sep):
            ...
            # fall through to LCFS
            out["reason"] = "separatrix_contour_failed"

        # LCFS-limiter fallback (robust)
        ...
        out["psi_sep"] = float("nan")  # not a separatrix in limiter mode
        if Rf is not None and Zf is not None and info.get("ok", False):
            ...
            # expose psi level used (useful for debugging)
