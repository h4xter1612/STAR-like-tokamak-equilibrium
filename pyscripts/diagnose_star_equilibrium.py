...
def _sample_vertical_axis_line(fields: Dict[str, Any], shape: Dict[str, Any]) -> Dict[str, np.ndarray]:
    """Sample magnetic field components along R = R_axis.

    This complements the midplane diagnostic. For up-down symmetric equilibria,
    B_R is expected to be near zero on the magnetic midplane, so a vertical cut is
    more informative for the radial-field structure.
    """
    R = fields["R"]
    Z = fields["Z"]
    R_ax, Z_ax = _get_axis(shape, None, R, Z, fields["psi"])
    ...
    if R.ndim == 2:
        i = int(np.nanargmin(np.abs(R[:, 0] - R_ax)))
        rline = R[i, :]
        zline = Z[i, :]
        idx = (i, slice(None))
    else:
        raise RuntimeError("Expected 2D R/Z arrays for vertical-axis sampling.")
...
def _write_vertical_axis_csv(path: Path, vert: Dict[str, np.ndarray]) -> None:
    rows = []
    Z = np.asarray(vert["Z"], float)
    for i in range(len(Z)):
        rows.append({
            "R_m": float(vert["R"][i]),
            "Z_m": float(vert["Z"][i]),
            "Br_T": float(vert["Br"][i]),
            "Bz_T": float(vert["Bz"][i]),
            "Bp_T": float(vert["Bp"][i]),
            "Btor_T": float(vert["Bt"][i]),
            "Btotal_T": float(vert["Btot"][i]),
        })
    _write_csv(path, rows)
...
def plot_equilibrium_clean(figdir: Path, eq: Any, geom: Dict[str, Any], shape: Dict[str, Any], fields: Dict[str, Any], args: argparse.Namespace) -> None:
    ...
    if args.plot_cad_target and "R_plasma" in geom and "Z_plasma" in geom:
        ax.plot(geom["R_plasma"], geom["Z_plasma"], lw=1.3, alpha=0.8, label="old CAD target / reference")
...
def plot_midplane_fields(figdir: Path, mid: Dict[str, np.ndarray], args: argparse.Namespace) -> None:
    ...
    panels = [
        ("Br", r"$B_R$ [T]", r"Radial field $B_R$ at magnetic midplane (symmetry check)"),
...
def plot_vertical_axis_fields(figdir: Path, vert: Dict[str, np.ndarray], args: argparse.Namespace) -> None:
    fig, axs = plt.subplots(2, 2, figsize=(11, 7), sharex=True)
    panels = [
        ("Br", r"$B_R$ [T]", r"Radial field $B_R$ along $R=R_{axis}$"),
        ("Bz", r"$B_Z$ [T]", r"Vertical field $B_Z$ along $R=R_{axis}$"),
        ("Bp", r"$B_p$ [T]", r"Poloidal field $B_p$ along $R=R_{axis}$"),
        ("Bt", r"$B_\phi$ [T]", r"Toroidal field $B_\phi$ along $R=R_{axis}$"),
    ]
    Z = vert["Z"]
    Zax = float(vert["Z_axis"][0])
    for ax, (key, ylabel, title) in zip(axs.ravel(), panels):
        ax.plot(Z, vert[key], lw=1.8)
        ax.axvline(Zax, ls=":", lw=1.5, label="magnetic midplane")
        ax.set_ylabel(ylabel)
        ax.set_title(title)
        ax.grid(True, alpha=0.30)
    ...
    axs[0, 0].legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(figdir / "vertical_axis_magnetic_fields.png", dpi=args.dpi, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument("--plot-cad-target", action="store_true", help="Overlay the old CAD plasma target as a reference curve in the overview plot")
    ...
    vert = _sample_vertical_axis_line(fields, shape)
    ...
    _write_vertical_axis_csv(outdir / "vertical_axis_fields.csv", vert)
    ...
    summary = {
        ...
        "field_diagnostic_note": "B_R on the magnetic midplane is a symmetry check and is expected to be near zero for an up-down symmetric equilibrium.",
    }
    ...
    plot_vertical_axis_fields(figdir, vert, args)
