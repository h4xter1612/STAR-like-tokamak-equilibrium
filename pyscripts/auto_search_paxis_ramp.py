...
# Último seed bueno: 9.40 MA El bueno es de 50 kPa .\results\highip_branch_search_20260614_083627\attempts\attempt_0001_seed_raw_best
seed = Path(
    r"results/highip_branch_search_20260613_160042/attempts/attempt_0012_seed_scaled_best_x1p25/best.json"
)
...
for paxis in paxis_stages:
    print("\n" + "=" * 100)
    print(f"[AUTO] Searching Ip={Ip_MA:.2f} MA, paxis={paxis:.3e} Pa from seed:")
    print(f"       {seed}")
    print("=" * 100)
    ...
    if ret != 0:
        print(f"[STOP] search_highip_branch_star.py failed at Ip={Ip_MA:.2f} MA and paxis={paxis:.3e} Pa")
        break

    if outdir is None:
        print(f"[STOP] Could not parse output directory at Ip={Ip_MA:.2f} MA and paxis={paxis:.3e} Pa")
        break

    valid = [c for c in accepted if valid_candidate(c)]

    if not valid:
        print(f"[STOP] No valid large-branch candidate found at Ip={Ip_MA:.2f} MA and paxis={paxis:.3e} Pa")
    ...
    print("\n" + "-" * 100)
    print(f"[ACCEPTED LARGE BRANCH] Ip={Ip_MA:.2f} MA, paxis={paxis:.3e} Pa")
